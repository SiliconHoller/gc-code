...
public class GnuplotCaps {
    ...
    public static void makeSrcPlot(ImageGraph graph, String basefile) throws IOException {
        int src = graph.getSrc();
        byte[] space = new String(" ").getBytes();
        byte[] nl = new String("\n").getBytes();
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        LinkedList<Integer> edges[] = graph.getEdges();
        LinkedList<Integer> ledges = edges[src];
        int pt = 0;
        int x = 0;
        int y = 0;
        double cap = 0.0;
        if (ledges != null) {
            Iterator<Integer> it = ledges.iterator();
            while (it.hasNext()) {
                pt = it.next();
                x = graph.getX(pt);
                y = graph.getY(pt);
                cap = graph.getAvailable(src, pt);
                baos.write(Integer.toString(x).getBytes());
                baos.write(space);
                baos.write(Integer.toString(y).getBytes());
                baos.write(space);
                baos.write(Double.toString(cap).getBytes());
                baos.write(nl);
            }
        }
        String nfile = basefile + "-"+Integer.toString(src)+".plt";
    ...
    public static void makeSinkPlot(ImageGraph graph, String basefile) throws IOException {
        int sink = graph.getSink();
        byte[] space = new String(" ").getBytes();
        byte[] nl = new String("\n").getBytes();
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        ...
        for (int i=0;i<edges.length;i++) {
            LinkedList<Integer> ledges = edges[i];
            if (ledges != null) {
                Iterator<Integer> it = ledges.iterator();
                while (it.hasNext()) {
                    pt = it.next();
        ...
        String nfile = basefile + "-"+Integer.toString(sink)+".plt";
        FileOutputStream fos = new FileOutputStream(nfile);
        fos.write(baos.toByteArray());
        fos.flush();
        fos.close();
    ...
    public static void main(String[] args) {
        try {
            ...
            GnuplotCaps.fillImageEdges(graph, imgSigma2);
            System.out.println("\tFilling Source edges...");
            GnuplotCaps.fillSourceEdges(graph,kavgs[0],srcSigma2);
            System.out.println("\tFilling Sink edges...");
            GnuplotCaps.fillSinkEdges(graph,kavgs[1],sinkSigma2);


            System.out.println("Creating plots of label availabilities...");
            makeSrcPlot(graph, file);
            makeSinkPlot(graph, file);
            
            System.out.println("Starting normal BK cut...");
            BorderCut mst = new BorderCut(graph);
            GnuplotCaps.cutAndResults(graph,mst,file,"bk");

            System.out.println("Creating post-cut plots...");
            makeSrcPlot(graph, file+"-postcut");
            makeSinkPlot(graph, file+"-postcut");

        } catch (IOException ex) {
            ex.printStackTrace();
            Logger.getLogger(GnuplotCaps.class.getName()).log(Level.SEVERE, null, ex);
        }
