...
import edu.ohio.graphcuts.alg.Scrubber;
 ...
 * @author david
 */
public class ScrubberComparison {
    ...
    public static void cutAndResults(ImageGraph graph, GraphCut gc, String file, String type) throws IOException {
        ...
        System.out.println("The cut took approximately "+(end-start)+" milliseconds.");
        ...
        //File cutsFile = new File(file+"-"+type+"C.png");
        File srcresultFile = new File(file+"-"+type+"S.png");
        File sinkresultFile = new File(file+"-"+type+"T.png");
    ...
    public static void main(String[] args) {
        try {
            ...
            KMeans kmeans = new XYGraySquaredKMeans(graph);
            ...
            ScrubberComparison.fillImageEdges(graph, imgSigma2);
            System.out.println("\tFilling Source edges...");
            ScrubberComparison.fillSourceEdges(graph,kavgs[0],srcSigma2);
            System.out.println("\tFilling Sink edges...");
            ScrubberComparison.fillSinkEdges(graph,kavgs[1],sinkSigma2);
            ...
            //EdmondsKarp bk = new EdmondsKarp(graph);
            //ScrubberComparison.cutAndResults(graph,bk,file,"unscrubbed-bk");
            
            System.out.println("Scrubbing...");
            //ImageGraph scrubGraph = pops.get4NGraph(img);
            //scrubGraph.setCapacities(caps);
            //scrubGraph.getNewFlows();
            Scrubber scrub = new Scrubber(graph);
            long start = System.currentTimeMillis();
            scrub.scrubGraph();
            long end = System.currentTimeMillis();
            System.out.println("Scrubbing took approximately "+(end-start)+" milliseconds.");
            BKTreeCut sbk = new BKTreeCut(graph);
            //EdmondsKarp sbk = new EdmondsKarp(scrubGraph);
            ScrubberComparison.cutAndResults(graph, sbk, file, "scrubbed-bk");
            System.out.println("Finished.");

        } catch (IOException ex) {
            ex.printStackTrace();
            Logger.getLogger(ScrubberComparison.class.getName()).log(Level.SEVERE, null, ex);
        }
