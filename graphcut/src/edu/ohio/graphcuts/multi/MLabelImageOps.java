 ...
 * @author david
 */
public class MLabelImageOps {


    public MLabelImageOps() {
    ...
    public MLabelImageGraph get4NGraph(BufferedImage img, int numLabels) {
        System.out.println("\tPngOps.get4NGraph():...");
        MLabelImageGraph graph = null;
        int w = img.getWidth();
        int h = img.getHeight();
        ...
        graph = new MLabelImageGraph(vertices,edges,labels,w,h);
        return graph;
    }

    public MLabelImageGraph getColor4NGraph(BufferedImage img, int numLabels) {
        MLabelImageGraph graph = get4NGraph(img, numLabels);
    ...
    public MLabelImageGraph get8NGraph(BufferedImage img, int numLabels) {
        MLabelImageGraph graph = null;
        int w = img.getWidth();
        int h = img.getHeight();
        int first = w*h;
    ...
    public BufferedImage labelImage(int label, GraphCut gc, MLabelImageGraph graph) {
        //init to all alpha=0
        ...
        for (int i=0;i<w;i++) {
            ...
            for (int j=0;j<h;j++) {
                ...
                int pos = w*j+i;
                if ( edges[pos] != null && gc.inLabel(label, pos)) {
    ...
    public static BufferedImage makeGrayScaleImage(BufferedImage img) {
        int w = img.getWidth();
        int h = img.getHeight();
        MLabelImageOps pops = new MLabelImageOps();
