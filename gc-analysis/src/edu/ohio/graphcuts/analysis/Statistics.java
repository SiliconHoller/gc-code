...
public class Statistics {
    ...
    public double getSigmaSquared(int clazz) {
        double sigma2 = 0.0;
        int count = getCount(clazz);
        System.out.println("count  for class "+clazz+" = "+count);
        double[] avg = getAverageFeatures(clazz);
        double[] devs = new double[count];
        int i = 0;
        int j = 0;
        while (i<count && j < classes.length) {
            if (classes[j] == clazz) {
                devs[i] = euclidianDistSquared(features[j],avg);
                i++;
            }
    ...
    public double euclidianDistSquared(int findex, double[] avg) {
        double dist2 = -1.0;
        double[] f = features[findex];
        if (f.length > 0) {
            dist2 = euclidianDistSquared(f,avg);
        }
        return dist2;
    }

    public double euclidianDistSquared(int fi, int fj) {
        double dist2 = -1.0;
        double[] ffi = features[fi];
        double[] ffj = features[fj];
        if (ffi.length > 0 && ffj.length > 0) {
            dist2 = euclidianDistSquared(ffi,ffj);
        }
        return dist2;
    }
    ...
    public static double getImageAverage(NLabelImageGraph ngraph) {
        double avg = 0.0;
        int[] verts = ngraph.getVertices();
        LinkedList<Integer>[] edges = ngraph.getEdges();
        int sum = 0;
        int count = 0;
        int len = ngraph.getWidth()*ngraph.getHeight();
        for (int i=0;i<len;i++) {
            if (edges[i] != null) {
                if (edges[i].size() > 0) {
                    sum += verts[i];
    ...
    public static double getImageSigmaSquared(NLabelImageGraph ngraph) {
        double avg = getImageAverage(ngraph);
        int[] verts = ngraph.getVertices();
        LinkedList<Integer>[] edges = ngraph.getEdges();
        int n=0;
        double sig2 = 0.0;
