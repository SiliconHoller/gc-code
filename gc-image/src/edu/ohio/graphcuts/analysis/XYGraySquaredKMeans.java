...
public class XYGraySquaredKMeans implements KMeans {
    ...
    public XYGraySquaredKMeans(NLabelImageGraph ngraph) {
        processGraph(ngraph);
    }
    ...
    private void processGraph(NLabelImageGraph ngraph) {
        numFeatures = 3;
        int[] verts = ngraph.getVertices();
        LinkedList<Integer>[] edges = ngraph.getEdges();
        int w = ngraph.getWidth();
        int h = ngraph.getHeight();
        features = new double[w*h][0];
        for (int i=0;i<features.length;i++) {
            if (edges[i] != null) {
                double[] f = new double[3];
        ...
        int numLabels = ngraph.getLabels().length;
        averages = new double[numLabels][3];
